...
The repository is currently being refactored from an initial research script into a modular and maintainable scientific code.
...
This avoids hard-coded indices such as U[5294,:,:].

## Output files

Simulation outputs are written to the outputs/ directory, which is ignored by Git.

A typical output directory contains time_series.csv, metadata.json, and summary.txt.

## Command reference

A full command reference is available in COMMANDS.md.

## Refactor notes

Detailed refactor notes are available in README_refactor.md.
