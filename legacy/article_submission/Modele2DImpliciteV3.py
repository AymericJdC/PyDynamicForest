...
def topHeight (V):
    ...
    # Sort by decreasing DBH, then by decreasing height
    cells.sort(key=lambda z: (z[0], z[1]), reverse=True)
    totaltrees=0.
    height_sum=0.
...
plt.ylabel("number of trees per ha")
...
data_age = np.array([18, 22, 28, 33, 38, 43, 45, 51, 53, 54, 57, 60, 63, 66, 69])

data_N = np.array([
    1924.34, 1907.89, 1825.66, 1669.41, 1381.58,
    1175.99, 1143.09, 978.62, 937.50, 921.05,
    921.05, 888.16, 855.26, 731.91, 682.57
])

plt.figure(figsize=(6, 4))

plt.plot(age, mass, label="Simulation")
plt.scatter(data_age, data_N, marker="o", label="Data")

plt.xlabel("time (years)")
plt.ylabel("number of trees per ha")
plt.title("Total number of trees per ha")
plt.grid()
plt.show()
...
data_Ho = np.array([
    11.66, 14.79, 19.25, 23.88, 26.77,
    29.83, 31.61, np.nan, np.nan, 36.84,
    np.nan, 38.85, 39.58, 42.24, 43.14
])

plt.figure(figsize=(6, 4))

plt.plot(age, ValueTopHeight, label="Simulation")
plt.scatter(data_age, data_Ho, marker="o", label="Data")
...
data_G = np.array([
    21.61, 31.87, 44.40, 50.92, 57.07,
    62.75, 66.32, 74.74, 75.14, 76.26,
    81.18, 85.33, 88.66, 87.93, 89.56
])
plt.figure(figsize=(6, 4))

plt.plot(age, ValueBasalArea, label="Simulation")
plt.scatter(data_age, data_G, marker="o", label="Data")
